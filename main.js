const MAX_RIVALS = 8,
    HEIGHT = 100,
    countSection = Math.floor(document.documentElement.clientHeight / HEIGHT); 
...
$audio.style.cssText = `width: 200px; height: 80px; position: absolute; top: 50px; left: 0; z-index: 100`;
...
const startGame = ({ target }) => {
    ...
    for (let i = 0; i < getQuantityElements(HEIGHT * settings.traffic); i++) {
        ...
        $rival.style.background = `transparent url(./image/enemy${randomRival}.png) center / cover no-repeat`;

        $gameArea.append($rival);
        
        $rival.style.left = `${Math.floor(Math.random() * ($gameArea.offsetWidth - $rival.offsetWidth))}px`;
    }
...
const playGame = () => {
    ...
    if (settings.start) {
        ...
        (keys.ArrowRight && settings.x < ($gameArea.offsetWidth - $car.offsetWidth)) 
            && (settings.x += settings.speed);

        (keys.ArrowUp && settings.y > 0) && (settings.y -= settings.speed);

        (keys.ArrowDown && settings.y < ($gameArea.offsetHeight - $car.offsetHeight)) 
...
const moveRival = () => {
    ...
    $rivals.forEach(rival => {
        ...
        if (rival.y >= $gameArea.offsetHeight) {
            //?? const checkTop = [...$rivals].every(item => item.offsetTop > HEIGHT);

            //?? checkTop && (rival.y = -HEIGHT * settings.traffic);
